...
const testarConversao = (fromValue, fromUnit, toUnit, valorEsperado) => {
  ...
  log(`${passou ? '✅' : '❌'} ${fromValue} ${fromUnit} → ${toUnit} = ${resultado} (esperado: ${valorEsperado})`);
...
const testarValidacaoEstoque = (estoqueDisponivel, unidadeEstoque, quantidadeSolicitada, unidadeSolicitada, resultadoEsperado) => {
  ...
  log(`${passou ? '✅' : '❌'} Estoque: ${estoqueDisponivel} ${unidadeEstoque}, Solicitado: ${quantidadeSolicitada} ${unidadeSolicitada} → ${resultado.valid ? 'Válido' : 'Inválido'} (esperado: ${resultadoEsperado ? 'Válido' : 'Inválido'})`);
...
async function main() {
  try {
    ...
    log("1. CONVERSÕES DE VOLUME (L e ML):");
    ...
    // Resumo dos testes
    log(`\n============ RESUMO ============`);
    log(`Total de testes: ${totais.testes}`);
    log(`Testes aprovados: ${totais.aprovados}`);
    log(`Taxa de sucesso: ${((totais.aprovados / totais.testes) * 100).toFixed(2)}%`);
    ...
    // Mensagem final com caminho do arquivo de log
